...
function createProducer(config: any, onDeliveryReport: any) {
...
async function produceExample() {
  ...
  const producer = await createProducer(config, (err: any, report: any) => {
    if (err) {
...
produceExample()
  ...
  // testing this: npx nodemon producer.ts getting-started.properties